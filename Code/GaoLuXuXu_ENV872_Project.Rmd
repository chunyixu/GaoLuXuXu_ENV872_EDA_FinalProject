...
Table: Dataset Structure Summary_1
...
\newpage
# Dataset Wrangling
Data preparation was carried out using the R programming language, with heavy use of the tidyverse, dplyr, and readxl packages. We removed incomplete rows and standardized missing values (e.g., '..') using na.omit() and na_if(). 

The dataset was then reshaped from wide to long format using pivot_longer(), with Year and Value columns created for time-series analysis. Only three indicators—CO$_2$ emissions, renewable energy consumption, and PM2.5 exposure—were retained via filtering.

As part of data cleaning, we confirmed that all numerical values were stored in appropriate formats. The Value column, initially read as a character type, was explicitly converted to numeric using as.numeric() to allow for statistical analysis.

Finally, the cleaned and reshaped dataset was saved as a separate .xlsx file using the write_xlsx() function for reproducibility and further use in exploratory data analysis, correlation testing, and regression modeling.


Table: Dataset Structure Summary_2

| Variable                          | Unit                          | Range           | Mean | Source               |
|----------------------------------|-------------------------------|------------------|--------------------------|----------------------|
| CO$_2$ Emissions                    | Metric tons per capita        | 1.4 – 17.4      | 8.3        | World Bank (WDI)     |
| Renewable Energy Consumption     | % of total energy use         | 7.4 – 32           | 16.9                   | World Bank (WDI)     |
| PM2.5 Pollution Exposure         | µg/m$_3$                         | 7.4 – 63.3      | 25.0       | World Bank (WDI)     |
...
```{r normalized.CO2 Emissions.country, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.height=5, fig.cap= "Normalized CO2 Emissions  (2010 = 1.0)"}
knitr::opts_chunk$set(echo = TRUE)
...
```{r normalized.Renewable Energy Consumption.country, echo=FALSE, message=FALSE, warning=FALSE,fig.align='left', fig.height=5, fig.cap= "Normalized Renewable Energy Consumption (2010 = 1.0)"}
# Normalized to 2010 values for comparison
...
```{r normalized.PM2.5 Pollution Exposure.country, echo=FALSE, message=FALSE, warning=FALSE,fig.align='left', fig.height=5, fig.cap= "Normalized PM2.5 Pollution Exposure (2010 = 1.0)" }
# Normalized to 2010 values for comparison
PM_normalized <- PM %>%
  group_by(`Country Code`) %>%
  mutate(Normalized = Value / first(Value))
...
```{r RQ1.1, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.height=5, fig.cap= "Cross Country Comparison of Renewable Energy Trends"}
knitr::opts_chunk$set(echo = TRUE)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.height=5, fig.cap= "Renewable Energy Adoption vs. CO2 Emissions (2010-2020)"}
# Scatterplot with regression line
...
```{r RQ2.1, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.height=5, fig.cap= "Cross Country Comparison of PM2.5 Air Pollution"}
...
### Scatterplot with Regression Line
```{r RQ2.4, echo=FALSE, message=FALSE, warning=FALSE, fig.align='left', fig.height=5, fig.cap= "CO2 Emissions vs. PM2.5 Pollution Exposure (2010-2020)"}
# Scatterplot
...
This study set out to explore the relationship between CO$_2$ emissions, renewable energy consumption, and PM2.5 air pollution in the context of sustainable development, using data from Germany, the United States, China, and Colombia between 2010 and 2020. These countries were selected to reflect a mix of developed and developing economies, offering comparative insights into how environmental outcomes vary by national context.

Our analyses reveal that renewable energy consumption is associated with reductions in CO$_2$ emissions in some countries but not uniformly across all. The United States and Germany showed strong, statistically significant negative correlations between renewable energy adoption and emissions, suggesting that targeted decarbonization policies, such as subsidies, carbon pricing, and coal phaseouts, can drive measurable climate benefits. In contrast, China and Colombia demonstrated weaker or even positive correlations, reflecting the complexity of balancing economic growth, energy demand, and emissions in transitioning economies.

When analyzing PM2.5 air pollution, we observed more mixed results. While PM2.5 exposure declined in all four countries during the study period, only the United States and Germany exhibited strong positive correlations between PM2.5 levels and CO$_2$ emissions. This suggests co-benefits of joint pollution and climate regulation. In contrast, China and Colombia showed weak or negative correlations, indicating that PM2.5 may be influenced by other sector-specific sources not directly tied to CO$_2$ emissions.

Importantly, our multi-variable models demonstrated that combining renewable energy consumption and PM2.5 exposure as predictors improved the explanatory power for CO$_2$ emissions, particularly in the United States and Germany, where over 75% of the variation in emissions was explained by these two variables. This reinforces the idea that climate and air quality policies can be most effective when integrated.

Overall, the findings suggest that while renewable energy and pollution controls are both vital, their impact depends on the country’s energy structure, policy environment, and development stage. Effective climate strategies must be multidimensional, combining renewable expansion with fossil fuel phaseout, industrial controls, and public health measures tailored to national contexts.

\newpage

# GitHub Repository
https://github.com/chunyixu/GaoLuXuXu_ENV872_EDA_FinalProject

\newpage

# References
1. World Meteorological Organization. “The Global Climate 2011-2020.” Wmo.int, 2023. https://library.wmo.int/records/item/68585-the-global-climate-2011-2020?offset=.
2. United Nations. (2023). *The Sustainable Development Goals Report 2023.* https://unstats.un.org/sdgs/report/2023/The-Sustainable-Development-Goals-Report-2023.pdf  
3. United Nations. (n.d). “The 17 Goals”. https://sdgs.un.org/goals  
4. The World Bank. “World Development Indicators.” The World Bank, 2024. https://databank.worldbank.org/source/world-development-indicators  
5. United Nations. “What Is Renewable Energy?” United Nations, 2023. https://www.un.org/en/climatechange/what-is-renewable-energy  
6. UNEP - UN Environment Programme. “Renewable Energy,” October 11, 2017. https://www.unep.org/topics/energy/renewable-energy/renewable-energy  
7. Brauer, M. et al. 2017, for the Global Burden of Disease Study 2017  
...
9. Qiu, Chengcheng. “PM2.5 rebounds in China in 2023, after falling for 10 years straight.” 22 Dec 2023. https://energyandcleanair.org/  
10. Anwar, M. N., et al. "Sources of carbon dioxide and environmental Issues." *Sustainable Agriculture Reviews* 37 (2019): 13–36.  
11. Saidi, K., & Omri, A. "Reducing CO2 emissions in OECD countries: do renewable and nuclear energy matter?" *Progress in Nuclear Energy*, 126 (2020): 103425.  
12. Liu, J.-Y., Woodward, R. T., & Zhang, Y.-J. "Has carbon emissions trading reduced PM2.5 in China?" *Environmental Science & Technology*, 55(10), (2021): 6631–6643.  
13. Gomez, Juan. “Colombia Can Lead the Energy Transition in Latin America.” *Atlantic Council*, May 6, 2022. https://www.atlanticcouncil.org/blogs/energysource/colombia-can-lead-the-energy-transition-in-latin-america/   
