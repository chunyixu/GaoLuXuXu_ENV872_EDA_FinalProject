...
label_data <- Renew[c(max_value, min_value), ]
...
model_COL_Q1 <- lm(CO2_COL$Index ~ Renew_COL$Index)
model_CHN_Q1 <- lm(CO2_CHN$Index ~ Renew_CHN$Index)
model_USA_Q1 <- lm(CO2_USA$Index ~ Renew_USA$Index)
model_DEU_Q1 <- lm(CO2_DEU$Index ~ Renew_DEU$Index)
...
# Scatterplot with regression line
ggplot(merged_data, aes(x = Index_Renew, y = Index_CO2, color = `Country Code`)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +  # Add regression line
  labs(
    title = "Renewable Energy Adoption vs. CO₂ Emissions (2000-2020)",
...
max_index <- which.max(PM$Index)
min_index <- which.min(PM$Index)
...
ggplot(PM, aes(x = Year, y = Value, group = `Country Code`, color = `Country Code`)) +
  ...
  labs(title = "Cross Country Comparison of PM2.5 Air Pollution", 
       x = "Year", y = "PM2.5 (µg/m³)") +
  geom_text(
    data = PM[c(max_value_Renew, min_value_Renew), ],
...
# Scatterplot
ggplot(merged_pm_data, aes(x = Index_CO2, y = Index_PM, color = `Country Code`)) +
  geom_point(size = 3, alpha = 0.7) +
